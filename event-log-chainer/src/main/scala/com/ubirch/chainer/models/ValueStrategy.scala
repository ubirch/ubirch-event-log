...
/**
  * Represents a Strategy for creating values objects. These are meant for the creation of lookup values.
...
/**
  ...
  */
case object SlaveTreeStrategy extends EventLogValueStrategy with LazyLogging {
  override def create(eventLog: EventLog): Seq[Value] = {

    if (eventLog.category == Values.UPP_CATEGORY) {
      ...
      Seq(Value(eventLog.id, Option(eventLog.category), Map.empty))
    }
...
/**
  ...
  */
case object OtherStrategy extends EventLogValueStrategy {
