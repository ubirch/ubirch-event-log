...
trait Api {
  ...
  def verifyUPPWithUpperBound(hash: Array[Byte],
                              @Query("response_form") responseForm: String = AnchorsNoPath.value,
                              @Query("blockchain_info") blockchainInfo: String = Normal.value): Future[Api.Response]

  @cors
  @CustomBody
  @POST("upp/verify/record")
  def verifyUPPWithUpperAndLowerBound(hash: Array[Byte],
                                      @Query("response_form") responseForm: String = AnchorsNoPath.value,
                                      @Query("blockchain_info") blockchainInfo: String = Normal.value): Future[Api.Response]
...
object Api extends VerificationServiceRestApiCompanion[Api] {
  ...
  object Response extends RestDataCompanion[Response] {
    ...
    implicit def asRestResp(implicit successAsRaw: AsRaw[HttpBody, Success],
                            failureAsRaw: AsRaw[HttpBody, Failure]): AsRaw[RestResponse, Response] = {
      AsRaw.create {
