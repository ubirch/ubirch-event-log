...
import com.ubirch.verification.microservice.services.{DefaultTestingGremlinConnector, Gremlin, LookupServiceBinder}
...
class LookupSpec extends TestBase with EmbeddedCassandra with LazyLogging {
  ...
  "Lookup Spec" must {
    ...
    "consume and process successfully when Found" in {
      ...
      val eventlogClientResponse: Future[EventLogClient.Response] = newEventLogClient.getEventByHash(value.getBytes, queryDepth, responseForm, blockchainInfo)
