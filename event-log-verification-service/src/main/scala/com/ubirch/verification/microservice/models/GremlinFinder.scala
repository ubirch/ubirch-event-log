...
import com.ubirch.verification.microservice.services.Gremlin
...
class GremlinFinder @Inject()(gremlin: Gremlin)(implicit ec: ExecutionContext) extends LazyLogging {
  ...
  def shortestPathFromVertexToBlockchain(hash: String): Future[List[Vertex]] = shortestPath(Values.HASH, hash, Values.PUBLIC_CHAIN_CATEGORY)
  ...
  def asVertices(path: List[Vertex], anchors: List[Vertex]): Future[(List[VertexStruct], List[VertexStruct])] = {
  ...
  def asVerticesDecorated(path: List[Vertex], anchors: List[Vertex]): Future[(List[VertexStruct], List[VertexStruct])] = {
  ...
  def getBlockchainsFromMasterVertexFromOption(master: Option[Vertex]): Future[List[Vertex]] =
    master match {
  ...
  case class PathHelper(path: List[Vertex]) {
    lazy val reversed: Seq[Vertex] = path.reverse
    lazy val headOption: Option[Vertex] = path.headOption
    lazy val reversedHeadOption: Option[Vertex] = reversed.headOption
    lazy val reversedTail: Seq[Vertex] =
      if (reversed.isEmpty) Nil
      else reversed.tail

    lazy val reversedTailReversed: Seq[Vertex] = reversedTail.reverse
    lazy val reversedTailHeadOption: Option[Vertex] = reversedTail.headOption
  }
