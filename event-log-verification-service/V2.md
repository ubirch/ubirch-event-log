...
The most important aspect of these endpoints is that we are introducing an 'ACCESS_TOKEN'. All the endpoints described
here must use a valid access token.

* [Access Token For Endpoints V2](AccessToken.md)

## (1) https://verify.demo.ubirch.com/api/v2/upp

This endpoint basically only queries for the existence of the upp. It checks that it has been stored on our backend. No
further checks are performed. You may think about this as a quick check.

_Example_

```shell script
curl -s -H 'Authorization: Bearer ACCESS_TOKEN' -X POST https://verify.demo.ubirch.com/api/v2/upp -d '24gpRTHckCjMTMxR17dBHxa8cKMm8uZ+I5HsFCOIbqU='
{
...
## (2) https://verify.demo.ubirch.com/api/v2/upp/verify

This query checks for the existence of the upp in our backend and additionally, it checks the "chain" and the validity
of the "keys" (That the UPP can be verified by one of the available keys for the particular device/entity.)

_Example_

```shell script
curl -s -H 'Authorization: Bearer ACCESS_TOKEN' -X POST https://verify.demo.ubirch.com/api/v2/upp/verify -d '24gpRTHckCjMTMxR17dBHxa8cKMm8uZ+I5HsFCOIbqU=' 
{
  "upp": "liPEEE9rZKelyUg3hsANMryOA8DEQHbKEbBFewQjK/9M8Df9wvDBLT9EkHIrtdyTs/LYhdega072sKOoFeGs4uq6Gfb986vjikaXiKdxeocZu/vUp6QAxCDbiClFMdyQKMxMzFHXt0EfFrxwoyby5n4jkewUI4hupcRAn8z4LHBlUtFORPDtp8bsQ4ANg+DogyAFLcsLqmW+28SWliB4Qmlv3Zglr/MH87UsIWR/+/Jx+ePhv/CSkSNVBg==",
...
## (3) https://verify.demo.ubirch.com/api/v2/upp/verify/anchor

This query checks for the existence of the upp in our backend, it checks the "chain" and the validity of the "keys" (
That the UPP can be verified by one of the available keys for the particular device/entity) and retrieves the upper
bounds or the closet blockchains transactions in the near future. You can get a compacted version or full version based
on the params below.
...
```shell script
curl -s -H 'Authorization: Bearer ACCESS_TOKEN' -X POST https://verify.dev.ubirch.com/api/v2/upp/verify/anchor -d 'oPV/aJsximYq2DbduTEarm8Jhae4uy61xOB6JIAACnFBCDJjJjBvz1sQNlqEfEAeCq1q5Kl1bv6KGz1y2wKQRw==' 
{
...
```shell script
curl -s -H 'Authorization: Bearer ACCESS_TOKEN' -X POST 'https://verify.dev.ubirch.com/api/v2/upp/verify/anchor?response_form=anchors_with_path&blockchain_info=ext' -d 'oPV/aJsximYq2DbduTEarm8Jhae4uy61xOB6JIAACnFBCDJjJjBvz1sQNlqEfEAeCq1q5Kl1bv6KGz1y2wKQRw=='
{
    "upp": "lSLEEOl+FgxhF1uJrJgVrrUmVeAAxECg9X9omzGKZirYNt25MRqubwmFp7i7LrXE4HokgAAKcUEIMmMmMG/PWxA2WoR8QB4KrWrkqXVu/oobPXLbApBHxEBQHgrfS+yaIC2XDoggA3R52cnWg2k1qkdDDS4iUUInZoIBd5MpsWGj55W5tcjVKFlP84T5Z2ocG0YdbSu7qjoM",
    "prev": null,
    "anchors": {
...
## (4) https://verify.dev.ubirch.com/api/v2/upp/verify/record

This query checks for the existence of the upp in our backend, it checks the "chain" and the validity of the "keys" (
That the UPP can be verified by one of the available keys for the particular device/entity) and retrieves the upper and
lower bounds or the closet blockchains transactions in the near future and past. You can get a compacted version or full
version based on the params below.
...
```shell script
curl -s -H 'Authorization: Bearer ACCESS_TOKEN' -X POST 'https://verify.dev.ubirch.com/api/v2/upp/verify/record' -d 'oPV/aJsximYq2DbduTEarm8Jhae4uy61xOB6JIAACnFBCDJjJjBvz1sQNlqEfEAeCq1q5Kl1bv6KGz1y2wKQRw=='
{
    "upp": "lSLEEOl+FgxhF1uJrJgVrrUmVeAAxECg9X9omzGKZirYNt25MRqubwmFp7i7LrXE4HokgAAKcUEIMmMmMG/PWxA2WoR8QB4KrWrkqXVu/oobPXLbApBHxEBQHgrfS+yaIC2XDoggA3R52cnWg2k1qkdDDS4iUUInZoIBd5MpsWGj55W5tcjVKFlP84T5Z2ocG0YdbSu7qjoM",
    "prev": null,
    "anchors": {
...
```shell script
curl -s -H 'Authorization: Bearer ACCESS_TOKEN' -X POST 'https://verify.dev.ubirch.com/api/v2/upp/verify/record?response_form=anchors_with_path&blockchain_info=ext' -d 'oPV/aJsximYq2DbduTEarm8Jhae4uy61xOB6JIAACnFBCDJjJjBvz1sQNlqEfEAeCq1q5Kl1bv6KGz1y2wKQRw=='
{
    "upp": "lSLEEOl+FgxhF1uJrJgVrrUmVeAAxECg9X9omzGKZirYNt25MRqubwmFp7i7LrXE4HokgAAKcUEIMmMmMG/PWxA2WoR8QB4KrWrkqXVu/oobPXLbApBHxEBQHgrfS+yaIC2XDoggA3R52cnWg2k1qkdDDS4iUUInZoIBd5MpsWGj55W5tcjVKFlP84T5Z2ocG0YdbSu7qjoM",
    "prev": null,
    "anchors": {
